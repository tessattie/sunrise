    ...
    <div class="row">
        ...
        <?php $sous = 0; $reductions = 0; $total = 0; $sous_us = 0; $reductions_us = 0; $total_us = 0; $volume=0; foreach ($sales as $sale): ?>
            <tr <?php if($sale->status == 0 || $sale->status == 4) : ?> style="background:#d9edf7" <?php endif; ?>>
                ...
                <td class="text-center"><?= $sale->has('customer') ? $this->Html->link(substr($sale->customer->first_name." ".$sale->customer->last_name, 0, 7)."...", ['controller' => 'Customers', 'action' => 'view', $sale->customer->id]) : '' ?></td>
