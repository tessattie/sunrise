...
class CategoriesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($ident = $this->Categories->save($category)) {
                $this->Flash->success(__('La catégorie a bien été sauvegardée'));

                return $this->redirect(['action' => 'edit', $ident['id']]);
            }
            $this->Flash->error(__('Nous n\'avons pas pu sauvegarder la catégorie. Réessayez plus-tard'));
        }
